...
class SyncSession {
    ...
    get config() {
        return this.withInternal((internal) => {
            const appUser = internal_1.binding.Helpers.syncUserAsAppUser(internal.user);
            if (!appUser) {
                throw new Error("User is null");
            }
            const user = new internal_1.User(appUser, mockApp);
     ...
     */
    get user() {
        const appUser = this.withInternal((internal) => internal_1.binding.Helpers.syncUserAsAppUser(internal.user));
        if (!appUser) {
            throw new Error("User is null");
        }
        return internal_1.User.get(appUser);
    }
    /**
